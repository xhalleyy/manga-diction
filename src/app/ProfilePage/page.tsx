                            ...
                            <div className='mt-4'>
                                ...
                                <div className={!showClubs ? favbox : noFavbox}>
                                        ...
                                        {ongoing.length === 0 ? <p className='col-span-5 h-64 text-xl font-poppinsMed italic text-darkbrown text-center py-10'>You have no favorited Mangas that you are currently reading.</p>
                                            ...
                                            (ongoing.map((manga, index) => (
                                                <div key={index} onClick={() => handleMangaClick(manga.manga.id)}>
                                                    <img className='w-[177px] h-64 rounded-lg py-1 cursor-pointer' src={manga.coverArtUrl} alt={manga.manga.title} />
                                                </div>
                                            )))}
                                    ...
                                    <div className='grid grid-cols-5 ms-5'>
                                        ...
                                        {/* finished reads */}
                                        {completed.length === 0 ? <p className=' col-span-5 h-64 text-xl font-poppinsMed italic text-darkbrown text-center py-10'>You have no favorited Mangas that you have completed.</p> : (completed.map((manga, index) => (
                                            <div key={index} onClick={() => handleMangaClick(manga.manga.id)}>
                                                <img className='w-[177px] h-64 rounded-lg py-1 cursor-pointer' src={manga.coverArtUrl} alt={manga.manga.title} />
                                            </div>
                                        )))}
